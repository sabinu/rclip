...
def main(tmp_datadir: str):
    ...
    ids_list, descriptions = zip(*IMAGENET2012_CLASSES.items(), strict=False)
    ...
    def process_batch():
        ...
        image_features = np.stack(
            [np.frombuffer(image["vector"], np.float32) for image in batch]
        )
        ...
        target_classes = np.array(
            [get_image_class(image["filepath"]) for image in batch]
        )
        top1_match += np.sum(target_classes == ids[ordered_predicted_classes[:, -1]])
        top5_match += np.sum(
            np.any(
                target_classes.reshape(-1, 1) == ids[ordered_predicted_classes[:, -5:]],
                axis=1,
            )
        )

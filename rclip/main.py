...
from rclip.utils.snap import check_snap_permissions, get_snap_permission_error, is_snap
...
class RClip:
    ...
    def __init__(
        ...
        self._image_regex = re.compile(
            f"^.+\\.({'|'.join(supported_image_ext)})$", re.I
        )

        excluded_dirs = "|".join(
            re.escape(dir) for dir in exclude_dirs or self.EXCLUDE_DIRS_DEFAULT
        )
        self._exclude_dir_regex = re.compile(
            f"^.+\\{os.path.sep}({excluded_dirs})(\\{os.path.sep}.+)?$"
        )
    ...
    def _index_files(self, filepaths: list[str], metas: list[ImageMeta]):
        ...
        try:
            ...
            return
        for path, meta, vector in cast(
            Iterable[PathMetaVector], zip(filtered_paths, metas, features, strict=False)
        ):
        ...
        with tqdm(total=None, unit="images") as pbar:
            ...
            for entry in fs.walk(directory, self._exclude_dir_regex, self._image_regex):
                ...
                if self._enable_raw_support:
                    ...
                    if (
                        file_ext in IMAGE_RAW_EXT
                        and self._does_processed_image_exist_for_raw(filepath)
                    ):
                        images_processed += 1
                        pbar.update()
                        continue
                ...
                try:
                    ...
                    print(
                        f"error getting fs metadata for {filepath}:",
                        ex,
                        file=sys.stderr,
                    )
                    continue
    ...
    def search(
        ...
        sorted_similarities = self._model.compute_similarities_to_text(
            features, positive_queries, negative_queries
        )
        ...
        filtered_similarities = filter(
            lambda similarity: (
                not self._exclude_dir_regex.match(filepaths[similarity[1]])
...
def main():
    ...
    try:
        result = rclip.search(
            args.query, current_directory, args.top, args.add, args.subtract
        )

...
class DB:
    ...
    """)
        # Query for images
        self._con.execute(
            "CREATE UNIQUE INDEX IF NOT EXISTS existing_images ON images(filepath) WHERE deleted IS NULL"
        )
        self._con.execute("CREATE TABLE IF NOT EXISTS db_version (version INTEGER)")
        self._con.commit()

    def ensure_version(self):
        db_version_entry = self._con.execute(
        ...
        if db_version_entry:
            ...
            self._con.execute(
                "INSERT INTO db_version(version) VALUES (?)", (self.VERSION,)
            )
        self._con.commit()
    ...
    def flag_images_in_a_dir_as_indexing(self, path: str, commit: bool = True):
        self._con.execute(
            "UPDATE images SET indexing = 1 WHERE filepath LIKE ?",
            (path + f"{os.path.sep}%",),
        )
        if commit:
            self._con.commit()
    ...
    def remove_indexing_flag(self, filepath: str, commit: bool = True):
        self._con.execute(
            "UPDATE images SET indexing = NULL WHERE filepath = ?", (filepath,)
        )
        if commit:
            self._con.commit()

    def get_image(self, **kwargs: Any) -> Image | None:
    ...
    def get_image_vectors_by_dir_path(self, path: str) -> sqlite3.Cursor:
        return self._con.execute(
            "SELECT filepath, vector FROM images WHERE filepath LIKE ? AND deleted IS NULL",
            (path + f"{os.path.sep}%",),
        )

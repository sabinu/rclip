...
QUERY_WITH_MULTIPLIER_RE = re.compile(
    r"^(?P<multiplier>(\d+(\.\d+)?|\.\d+|\d+\.)):(?P<query>.+)$"
)
...
class Model:
    ...
    def __init__(self, device: str = "cpu"):
        ...
        self._text_model_path = (
            helpers.get_app_datadir()
            / f"{self._model_name}_{self._checkpoint_name}_text.pth"
        )
        self._text_model_version_path = (
            helpers.get_app_datadir()
            / f"{self._model_name}_{self._checkpoint_name}_text.version"
        )
    ...
    def _load_model(self):
        self._model_var, _, self._preprocess_var = (
    ...
    def compute_image_features(
        ...
        images_preprocessed = torch.stack(
            cast(list[torch.Tensor], [self._preprocess(thumb) for thumb in images])
        ).to(self._device)
        with torch.no_grad():
    ...
    def compute_text_features(self, text: list[str]) -> npt.NDArray[np.float32]:
        ...
        with torch.no_grad():
            text_features = self._model_text.encode_text(
    ...
    def _group_queries_by_type(
        ...
        list[QueryWithMultiplier], list[QueryWithMultiplier], list[QueryWithMultiplier]
    ]:
    ...
    def compute_features_for_queries(self, queries: list[str]) -> FeatureVector:
        text_features: FeatureVector | None = None
        image_features: FeatureVector | None = None
        ...
        if files or urls:
            file_multipliers, file_paths = (
                cast(tuple[tuple[float], tuple[str]], zip(*(files), strict=False))
                if files
                else ((), ())
            )
            url_multipliers, url_paths = (
                cast(tuple[tuple[float], tuple[str]], zip(*(urls), strict=False))
                if urls
                else ((), ())
            )
            try:
                ...
                print(
                    f'File "{e.filename}" not found. Check if you have typos in the filename.'
                )
                sys.exit(1)
            except UnidentifiedImageError as e:
                print(
                    f'File "{e.filename}" is not an image. You can only use image files or text as queries.'
                )
                sys.exit(1)
            image_multipliers = np.array(url_multipliers + file_multipliers)
            image_features = np.add.reduce(
                self.compute_image_features(images) * image_multipliers.reshape(-1, 1)
            )

        if phrases:
            phrase_multipliers, phrase_queries = cast(
                tuple[tuple[float], tuple[str]], zip(*phrases, strict=False)
            )
            phrase_multipliers_np = np.array(phrase_multipliers).reshape(-1, 1)
            text_features = np.add.reduce(
                self.compute_text_features([*phrase_queries]) * phrase_multipliers_np
            )
    ...
    def compute_similarities_to_text(
        ...
        sorted_similarities = sorted(
            zip(similarities, range(item_features.shape[0]), strict=False),
            key=lambda x: x[0],
            reverse=True,
        )
